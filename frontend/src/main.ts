...
import {
  ...
  showNotice, renderDemoUserSelector,
} from "./ui.js";
...
async function loadSelects(): Promise<void> {
  try {
    ...
    renderDemoUserSelector(usResult.items, state.demoUserId, (id) => {
      state.demoUserId = id;
      setDemoUserId(id);
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.getElementById("registrationForm")!.addEventListener("submit", async (e) => {
    ...
    try {
      await createRegistration({ eventId: data.eventId, userId: state.demoUserId });
